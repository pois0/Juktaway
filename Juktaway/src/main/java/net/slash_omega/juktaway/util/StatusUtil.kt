...
val Status.displayText: CharSequence
    get() = SpannableStringBuilder().also { sb ->
        entities.run { userMentions + hashtags + urls + media }
                .sortedBy { it.firstIndex }
                .fold(0) { acc, entity ->
                    when (entity) {
                        is StatusEntity.UserMentionEntity -> {
                            val userMention = "@" + entity.screenName
                            val start = text.indexOf(userMention, acc, true).takeUnless { it < 0 } ?: return@fold acc
                            sb.append(text.substring(acc, start))
                            sb.append(userMention, UnderlineSpan(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                            start + entity.size
                        }
                        is StatusEntity.HashtagEntity -> {
                            val hashtag = "#" + entity.text
                            val start = text.indexOf(hashtag, acc, true).takeUnless { it < 0 } ?: return@fold acc
                            sb.append(text.substring(acc, start))
                            ...
                            val start = text.indexOf(entity.url, acc, true).takeUnless { it < 0 } ?: return@fold acc
                            sb.append(text.substring(acc, start))
